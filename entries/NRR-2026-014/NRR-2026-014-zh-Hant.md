...
這是「說一套、做一套」的直接實例——並非出於惡意，而是執行慣性所致。操作者在執行翻譯管線時，回到了「自己認為最快的方式」（單模型＋自檢），而不是 Spec 規定的「更嚴謹但更慢的方式」（多模型＋異後端校譯）。這揭示了 Spec Coding 的核心挑戰：Spec 的價值只有在執行中才會受到檢驗，而執行者天生就有「走捷徑」的傾向。框架主張的 Human Gate 機制原本應在此攔截偏離——Human Gate 要求在執行偏離 Spec 時停下來確認；但操作者（同時也是 Human Gate）在「效率優先」的心態下跳過了這個 Gate。
