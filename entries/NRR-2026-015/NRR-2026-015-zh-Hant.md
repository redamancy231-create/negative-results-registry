...
這三次誤報並非單純因為操作不仔細，而是同一後端與同一搜尋策略造成的結構性盲區。操作者用自己撰寫的 grep pattern 掃描自己編輯過的檔案；大腦在編輯過程中已建立「我已經處理過了」的認知，grep 只是再次確認這項認知。異後端的獨立掃描採用不同的搜尋策略，包括不同的關鍵字選擇與不同的檔案範圍假設，因此自然會命中操作者盲區中的殘留。第四次修復後，專案啟動檢查清單新增 O7 `no_self_reported_zero_residue`：清理類任務不得依據自有 grep pattern 宣稱零殘留，必須由異後端或異工具獨立確認。
