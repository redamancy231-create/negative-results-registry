...
> v0.1 · 基于 v0.1.0（22 条目）分析 · 生成模型：DeepSeek-V4-Pro (via Claude Code CLI) · 2026-07-25
...
**独立价值**：22 条目的结构可以直接当模板——复制格式、改内容、用 CI 校验。
...
| 方向 | 说明 | 瓶颈 |
|------|------|------|
| **多语言条目** | 将 22 条目的 .md 文件翻译为英文/正體中文 | 翻译成本（22 × 2 = 44 份文件） |
...
| 效应量模式聚类 | ⚠️ n=22 偏小 |
| 教训主题分类 | ✅ |
| 第一方 vs 第三方差异 | ⚠️ n=22 偏小，15 vs 7 |
| 跨条目模式识别（P1-P8 类） | ❌ n=22 不够（已知结论——methodology-extraction-methodology G5 审计） |

**局限**：22 条目全部来自同一作者——不能泛化到其他操作者。等条目数 ≥50 且包含 ≥3 独立提交者后方可做跨操作者分析。
...
| 7 | **不要在 README 中声称"社区登记册"如果没有外部提交**——当前 22 条目全部来自维护者 | 见 v0.1.0 README "当前为维护者个人失败日志的结构化原型" |
